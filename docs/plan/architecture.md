...
下图概括 OPC 的三层结构、主链路角色与运行支撑设施之间的关系。组织层负责"谁做什么"，执行基础设施层提供"工作台"，评估与运维闭环层保证"质量与可恢复"。

```mermaid
flowchart TB
    User([用户 / 单人操作者])

    subgraph ORG["组织层 (MetaGPT 式多角色协同)"]
        direction LR
        PM[PM<br/>需求/PRD]
        ENG[Engineer<br/>实现/单测]
        QA[QA<br/>验收/退回]
        ARCH[Architect]
        OPS[Ops]
        GROW[Growth]
    ...
    subgraph INFRA["执行基础设施层 (OpenHands/OpenDevin 式能力抽象)"]
        direction LR
        TOOLS[工具系统<br/>ToolDefinition/permission]
        MEM[记忆与上下文<br/>WorkingMemory/RAG]
        KB[(知识检索<br/>BM25/向量)]
        PERM[权限 Profile<br/>危险命令策略]
        PLUG[插件 / MCP<br/>manifest 安全契约]
    end

    subgraph LOOP["评估与运维闭环层 (Harness 运行机制)"]
        direction LR
        PLAN[任务规划<br/>长任务恢复]
        TRACE[Run Trace<br/>events/trace/metrics]
        COST[成本观测<br/>opc runs cost]
        GUARD[Guardrail<br/>校验/审批/恢复]
        HUMAN[人工干预节点]
    end
...
P8 已落地的 agent 资产样板：[.claude/agents/pm.md](.claude/agents/pm.md)、[.claude/agents/engineer.md](.claude/agents/engineer.md)、[.claude/agents/qa.md](.claude/agents/qa.md)，索引见 [.claude/agents/README.md](.claude/agents/README.md)。
...
### 7.0 P10 插件 Trust Policy

| 来源 | 信任等级 | 默认加载 | Release gate 要求 |
| --- | --- | --- | --- |
| 内置插件 | high | 仍需 manifest 显式声明 | 字段、权限、路径和测试通过 |
| 仓库插件 | medium | 默认禁用，配置启用 | manifest 位于项目根内；权限不得越界；重复 id 拒绝 |
| 外部插件 | low | 默认不加载 | 需要人工审批；后续应补 hash/签名或固定版本 |

插件 manifest 的默认规则：来源可追踪、版本/路径可定位、权限最小化、禁用/卸载可通过配置完成、兼容矩阵写入 release-check。Project type manifest 路径越出项目根、包含未知字段、重复 id 或声明未知权限时必须拒绝加载，不以 warning 继续。
...
Qt 生成相关文档按“需求 → 架构 → 运行流程 → 用户入口”分层维护：

| 入口 | 用途 |
| --- | --- |
| [qt-generation-prd.md](qt-generation-prd.md) | 说明 P9 Qt 5.14.2 Widgets + CMake 的产品范围、非目标、环境前提和验收口径 |
| [architecture.md#75-qt-环境检测规则](architecture.md#75-qt-环境检测规则) | 说明插件默认禁用、显式启用、延迟检查、Qt 环境诊断字段和关闭插件边界 |
| [../workflow-packs/qt-generation.md](../workflow-packs/qt-generation.md) | 说明 Qt 生成 workflow pack 的输入、输出、权限、构建验证、QA 验收和 trace/artifact 要求 |
| [../../README.md#qt-项目生成可选插件](../../README.md#qt-项目生成可选插件) | 面向用户的 quickstart：启用 Qt 插件、执行 `opc project-types list`、`opc generate qt`、CMake 构建和 `opc trace inspect` |
| [../../plugins/qt/opc-plugin.toml](../../plugins/qt/opc-plugin.toml) | Qt plugin manifest source of truth：`project_type=qt`、`widgets-cmake` 模板、环境检查和构建命令 |

## 8. OPC Harness L1-L6 能力矩阵

| 层级 | OPC 当前能力 | 阶段进展 |
| --- | --- | --- |
| L1 信息边界 | `ROLE_CONTEXT_SECTIONS` 按角色裁剪 ContextPack，RAG 来源通过 `context_sources` 标注；memory 注入冲突时 `conflict_current_fact` 标记 | P6 产品化为上下文治理契约；P8 补 memory 来源追踪与冲突回退 |
| L2 工具系统 | ToolDefinition 声明 permission、side_effect、timeout，支持 profile 过滤；插件 manifest 与 workflow pack manifest 已落地 | P6 加权限 profile 与插件 manifest；P8 落地 `docs/workflow-packs/` 与 manifest 模板 |
| L3 执行编排 | PM → Engineer → QA 主链路，动态 Architect/Ops/Growth，run trace 记录阶段事件；可选角色触发可显式覆盖 | P6 保持 MVP 工作流；P8 文档化触发词与 CLI 手动开关 |
| L4 记忆与状态 | WorkingMemory、workflow state、artifacts、run trace 分离；memory 候选筛选与 `memory_write_policy` 审计事件 | P6 明确 run 状态不写入长期 memory；P8 实现复盘候选筛选、supersede/删除路径与审计 |
| L5 评估与观测 | run_metrics、run_events、run_trace、QA 输出 schema；分阶段 model / input_tokens / output_tokens / estimated_cost；`opc runs cost` 趋势 CLI | P6 增加 trace inspect 与 golden 样例；P8 完成成本字段、聚合函数与只读趋势命令 |
| L6 约束校验与恢复 | schema 校验、max rounds、dangerous command guardrail、resume；长任务 `id/depends_on/read_before_start/execution/evidence/handoff` 字段与执行游标 | P6 加强 GuardrailPolicy 与审批事件；P8 补长任务恢复纪律与跨会话继续协议 |

结论：OPC 已具备 harness 的主干骨架，P6 把已有能力产品化和可验证化，P8 进一步补齐成本观测闭环、知识回流边界、长任务恢复机制和资产层索引；暂不复制 DeerFlow 的宽 agent 编排、长期自治 memory 或服务化控制面。
...
因此，后续阶段（P7 之后）的改进优先围绕本地项目 registry、run 冲突提示、memory 生命周期、数据清理/备份和只读审计导出展开；团队治理、多用户模式和服务化部署需要单独立项，不应混入 v1 默认承诺。
...
| --- | --- | --- | --- | --- |
| 敏感数据 | 只有用户任务、工具或检索范围显式触达时读取 | 不应写入长期 memory；trace 中只保留必要诊断摘要 | 仅在模型上下文或外部工具调用需要时，且必须受权限/来源约束 | 后续阶段补敏感扫描、redaction 和清理命令 |
...
| 日志与 trace | 工具调用、阶段输出、guardrail 事件会被记录 | `run_events.jsonl`、`run_trace.json`、`run_metrics.json` | 默认不上传；外部审计导出需用户显式触发 | 默认本地保留，后续阶段补按 run/project 清理策略 |
...
| Memory | 只读取与任务、角色、scope 匹配且未过期的长期事实 | 用户偏好、项目决策、外部引用可长期保存 | 不自动发送全量 memory，只注入必要片段与来源 | P8 已实现写入审核、删除/supersede 与 `memory_write_policy` 审计事件 |
| 审计保留 | 本地审计用于复盘、失败定位和安全检查 | 归属本地操作者，不作为团队问责系统 | 默认不出域；导出需用户显式命令 | 后续阶段补只读导出、保留期和清理策略 |
...
P8 把成本观测纳入评估与运维闭环层的稳定字段，目标是让单次 run 的资源消耗可只读复盘，多次 run 的趋势可聚合分析。

| 字段层级 | 字段 | 来源 | 缺失行为 |
| --- | --- | --- | --- |
| 阶段 | `model`、`input_tokens`、`output_tokens`、`api_calls`、`duration_seconds`、`estimated_cost`、`currency`、`pricing_source` | Agent usage 与阶段日志 | 缺 usage 时安全落空，估算成本置零并标注 `pricing_source=unknown` |
| 总计 | `totals.input_tokens`、`totals.output_tokens`、`totals.estimated_cost`、`totals.duration_seconds` | 阶段聚合 | 缺阶段字段时按已知部分聚合，不阻断 trace 读取 |
| 趋势 | 多次 run 按 `run_id` 聚合，提供 stages/totals/compatibility 三段输出 | `aggregate_run_cost_trend` | 旧 metrics 缺字段时进入 compatibility 段说明 |

只读入口：`opc runs cost --limit N` 展示最近 N 次 run 的明细、阶段聚合和兼容性提示，不调用模型也不修改 artifacts。`token-report` skill 基于同一份 metrics 输出单次 run 的报告，禁止重新调用模型。

成本估算配置由 `CostConfig` 控制：用户可关闭估算、覆盖模型价格或切换 currency；未配置价格的模型不阻断流程，只在 metrics 中标注 `pricing_source=unknown`。

## 15. 长任务执行与跨会话恢复机制

P8 把长任务恢复纳入 L6 约束校验与恢复层，目标是让多阶段任务在上下文压缩或换会话后仍能从文件和产物恢复，而不依赖聊天历史。

任务字段标准（详见 [docs/claude/standards.md](../claude/standards.md)）：

| 字段 | 用途 | 必填情况 |
| --- | --- | --- |
| `id` | 跨会话引用、commit message、依赖标注的稳定标识 | 长任务必填 |
| `depends_on` | 显式列出前置任务 ID，避免只用自然语言描述依赖 | 有前置依赖时必填，无依赖写 `none` |
| `read_before_start` | 继续任务前必须读取的任务条目、依赖产物、相关文件、pending diff 或验收记录 | 长任务必填 |
| `execution` | 区分 main 会话、Explore subagent、skill、manual_review、qa_acceptance | 长任务必填 |
| `evidence` | 产物路径、验证方式、验证结果或未验证原因 | 任务进入完成状态前必填 |
| `handoff` | 当前状态、下一步、阻塞项、需重读文件、最近验证结果 | 阶段性压缩或换会话前必填 |

任务文件顶部维护"执行游标"：当前任务、下一任务、阻塞项、最近验证、继续方式、继续前必须读取的文件清单。清空上下文后必须先读取游标和任务条目，再进入实现，禁止依赖聊天历史。

执行主体边界：Explore subagent 只用于独立探索、检索、只读 review，不作为长期上下文保存器；skill 可生成、自检或验收，不直接执行高风险动作；main 会话承担实现与提交。任何长任务进入 QA 前必须经过 implementation-check 自检，自检失败时回退 Engineer，不得直接进入 QA。
