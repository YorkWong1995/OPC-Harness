...
| 步骤 | 责任角色 | 输入 | 输出 | 继续 / 完成条件 | 回退条件 | 升级条件 |
| --- | --- | --- | --- | --- | --- | --- |
| 1 需求结构化 | PM | 用户目标、业务问题、约束条件 | PRD、范围说明、验收标准 | 目标明确、范围可控、验收标准可检查 | 目标模糊、边界不清、缺少关键约束 | 目标冲突、优先级冲突、范围明显失控 |
| 2 方案判断 | Architect 或 PM | PRD、现有约束、已有系统信息 | 架构说明或简化实现方案 | 存在可执行路径，复杂度与 MVP 匹配 | 方案依赖不明、实现路径不清 | 出现高影响技术取舍，需要用户确认 |
| 3 任务拆解 | Planner、PM 或 Architect | PRD、方案说明 | 任务清单、责任角色、依赖关系、完成标准 | 任务可执行、顺序清晰、完成标准明确 | 任务粒度过大、依赖不清、责任不明 | 拆解后发现范围超出 MVP |
| 4 实现执行 | Engineer | 任务清单、方案说明、代码上下文 | 代码变更、实现说明、最小验证结果 | 实现完成且具备最小验证证据 | 实现被阻塞、方案无法落地、上下文缺失 | 需要改动超出原范围，或触发高风险操作 |
| 5 验证验收 | QA | PRD、验收标准、代码变更、验证结果 | 验收记录、缺陷清单、通过/退回结论 | 验收通过，关键标准满足 | 存在功能缺陷、验收失败、验证证据不足 | 验收标准与实现结果冲突，需要 PM 或用户裁决 |
| 6 复盘沉淀 | PM、QA 或 CEO / 用户 | 任务结果、验收结论、执行问题、日志记录 | 复盘记录、经验总结、流程修正项 | 明确本轮是否成功、问题在哪里、下轮如何改进 | 无 | 若暴露系统性问题，升级为流程或规范修订任务 |
...
- 回退不是无限循环：QA 返工默认上限为 `max_rework_attempts`（默认 1 次），整体轮次上限为 `max_rounds`（默认 12）；超出即触发熔断（`circuit_breaker_open`）暂停工作流，交人工接管。详见第 12 节。
...
### 9.5 P10 Runtime Workflow Pack Manifest Schema

P10 将 workflow pack manifest schema 落到 `src/opc/workflow_spec.py`，用于只读发现、校验和低风险 smoke。最小字段仍为 `id`、`kind`、`owner_roles`、`inputs`、`outputs`、`permissions`、`acceptance`、`trace`；runtime manifest 可选补充 `stages` 与 `transitions`。

| 字段 | 校验规则 |
| --- | --- |
| `id` | 稳定 pack id，和文档文件名保持一致 |
| `kind` | 只能是 `claude_skill`、`opc_runtime_workflow` 或 `documentation` |
| `owner_roles` | 至少能定位责任角色，不替代角色输出 schema |
| `permissions` | 只能使用已知权限 token；未知权限必须拒绝进入 runtime |
| `stages` | 使用 `WorkflowStage` 字段描述阶段名、状态、可选角色和审批要求 |
| `transitions` | transition 的 from/to state 必须存在于 stages；未知 condition 拒绝 |
| `trace` | 声明必须写入 run trace 或 artifact 的关键事件 |

`opc workflow-packs list` 只读列出内置/文档 pack、kind、权限、runtime 可执行性和禁用原因，不触发模型或工具执行。`opc workflow-packs smoke --id docs-update` 只执行低风险 smoke：创建 run_id、`workflow_pack_smoke.json` 和 trace，证明 pack 可被 runtime 发现与记录；不执行模型调用、不修改业务代码、不触发真实发布。
...
### 13.4 Cost trend 输出格式

`opc runs cost` 用于只读聚合最近 N 次 run 的 `run_metrics.json`，默认从 workspace 下的 artifacts 目录发现 run，不调用模型、不重跑 workflow、不写入状态。

按 run 输出字段：

| 字段 | 含义 |
| --- | --- |
| `run_id` | run 标识，来自 trace 或 metrics |
| `status` | run 最终状态，缺失时显示 `unknown` |
| `input_tokens` / `output_tokens` | run 总输入/输出 token |
| `estimated_cost` / `currency` | 本地估算成本和币种，未知时显示 `unknown` |
| `duration_seconds` | run 总耗时 |
| `pricing_source` | 价格来源，缺失或部分缺失时显示 `unknown` / `partial` |
| `artifacts_dir` | metrics 所在 artifacts 目录 |

按阶段/角色聚合字段：`stage`、`runs`、`input_tokens`、`output_tokens`、`estimated_cost`、`duration_seconds`、`api_calls`、`tool_calls`。旧 metrics 缺少 model/cost/stage 字段时必须保留 token/duration 可读部分，并在 compatibility 中标注缺失项。
...
实现阶段完成后，进入 QA 前必须按顺序检查：

1. Engineer 输出实现摘要、受影响文件、验证方式和已知限制。
2. 执行 `implementation-check` 或等价自检，确认任务范围、任务字段、pending diff、验证证据和上下文可恢复性。
3. 自检结论为“建议进入 QA”时，才进入 QA / acceptance-check；结论为“不建议进入 QA”时，回到 Engineer 修正，不生成 QA 通过结论。
4. QA 依据验收标准、实现证据和可恢复性证据给出 pass/fail；缺少验证证据或 handoff 时不能通过。

### 16.2 发布类任务发布前门禁

发布、部署、回滚、运维类任务在 QA 通过后，还必须执行发布前检查：

1. 确认 QA 结论为 pass，且发布范围、目标环境和运行验证方式明确。
2. 执行 `release-check` 或等价检查，列出发布前检查项、回滚条件、运行验证和监控关注点。
3. 对会修改远端、发布环境、CI/CD 或共享状态的动作，必须获得人工确认后执行。
4. 发布后必须记录运行验证结果；触发回滚条件时停止后续动作并交给 Ops / human 决策。
