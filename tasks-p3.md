...
- [ ] 检查 README 中所有 API 示例是否与实际代码一致 <!-- files: README.md, src/opc/ --> <!-- context: 移除或修正不存在的 API，例如未实现的 opc.run_as_role() -->
...
- [ ] 校验 send_to 和 cause_by 字段合法性 <!-- files: src/opc/environment.py, src/opc/schema.py --> <!-- context: 防止消息路由到不存在角色或触发非法状态流转 -->
- [ ] 实现动态角色激活策略 <!-- files: src/opc/workflow.py, src/opc/roles.py --> <!-- context: 默认只启用 PM/Engineer/QA，Architect/Ops/Growth 按任务类型或人工选择启用 -->

## 4. 端到端工作流测试

- [ ] 添加 PM → Engineer → QA 正常通过的端到端测试 <!-- files: tests/ --> <!-- context: 使用固定输入和 mock LLM response，验证状态流转、产物生成、最终通过 -->
- [ ] 添加 QA 打回 Engineer 的端到端测试 <!-- files: tests/ --> <!-- context: 验证 QA fail 后能够回流 Engineer 并再次验收 -->
...
- [ ] 添加角色输出契约测试 <!-- files: tests/ --> <!-- context: 验证 PM/Engineer/QA 输出缺字段、格式错误时会被拒绝 -->
