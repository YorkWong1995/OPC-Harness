...
- [ ] LT-07 更新 task-spec 字段要求 <!-- files: .claude/skills/task-spec/SKILL.md --> <!-- context: 责任角色=PM/Engineer；输入=LT-01 到 LT-06 的字段标准、现有 task-spec 输出格式；输出=task-spec 生成任务时支持 id、depends_on、read_before_start、execution、evidence、handoff；依赖关系=依赖 LT-01 到 LT-06；完成标准=新生成任务默认包含长任务可恢复字段，不再只依赖 files/context 注释 --> <!-- order: 依赖字段标准完成 -->
