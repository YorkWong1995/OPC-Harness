...
- [ ] LT-09 更新 task-spec 依赖生成规则 <!-- files: .claude/skills/task-spec/SKILL.md --> <!-- context: 责任角色=PM/Architect；输入=depends_on、order、review、auto 的关系；输出=依赖任务生成规则；依赖关系=依赖 LT-07；完成标准=skill 能在生成任务时显式标注前置任务 ID，而不是只写自然语言依赖 --> <!-- order: 依赖 task-spec 字段要求 -->
