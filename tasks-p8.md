...
- [ ] LT-04 定义任务执行主体字段 <!-- files: docs/claude/standards.md --> <!-- context: 责任角色=Architect；输入=main 会话、Explore subagent、skill、QA review 的边界；输出=execution 字段取值和适用场景；依赖关系=依赖 LT-01；完成标准=标准中能区分 main、Explore subagent、skill、manual review、QA acceptance 的使用边界 --> <!-- order: 依赖长任务字段标准 -->
