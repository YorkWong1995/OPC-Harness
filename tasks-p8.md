...
- [ ] LT-14 更新 implementation-check 范围检查 <!-- files: .claude/skills/implementation-check/SKILL.md --> <!-- context: 责任角色=QA/Engineer；输入=新任务字段、pending diff、任务 evidence；输出=implementation-check 新增任务字段完整性检查；依赖关系=依赖 LT-07；完成标准=实现自检会检查任务 ID、依赖、执行主体、完成证据是否存在或说明缺失原因 --> <!-- order: 依赖 task-spec 字段要求 -->
- [ ] LT-15 更新 implementation-check 上下文恢复检查 <!-- files: .claude/skills/implementation-check/SKILL.md --> <!-- context: 责任角色=QA；输入=read_before_start、handoff、依赖产物；输出=上下文恢复检查项；依赖关系=依赖 LT-14；完成标准=自检能判断任务是否可在清空聊天上下文后由文件恢复 --> <!-- order: 依赖 implementation-check 字段检查 -->
- [ ] LT-16 更新 acceptance-check 验收字段 <!-- files: .claude/skills/acceptance-check/SKILL.md --> <!-- context: 责任角色=QA；输入=任务 evidence、验收文档字段、任务完成状态；输出=acceptance-check 对 evidence 和 handoff 的验收要求；依赖关系=依赖 LT-05 和 LT-06；完成标准=验收报告会指出缺少验证证据、交接信息或前置读取记录的问题 --> <!-- order: 依赖 evidence 和 handoff 标准 -->
- [ ] LT-17 更新 acceptance-check 输出示例 <!-- files: .claude/skills/acceptance-check/SKILL.md --> <!-- context: 责任角色=QA；输入=更新后的验收字段；输出=包含任务可恢复性结论的验收示例；依赖关系=依赖 LT-16；完成标准=示例包含“可清空上下文继续”和“不可清空上下文继续”两种结论 --> <!-- order: 依赖 acceptance-check 字段更新 -->
...
- [ ] LT-25 更新任务系统索引说明 <!-- files: README.md, docs/DOCS_STRUCTURE.md --> <!-- context: 责任角色=PM；输入=更新后的 standards、discipline、task-spec、implementation-check、acceptance-check；输出=长任务拆解与执行机制入口说明；依赖关系=依赖 LT-01 到 LT-17；完成标准=用户能从 README 或 DOCS_STRUCTURE 找到长任务规则、任务生成 skill 和验收 skill --> <!-- order: 依赖核心规范与 skill 更新 -->
- [ ] LT-26 执行长任务机制自检 <!-- files: docs/claude/standards.md, docs/claude/discipline.md, .claude/skills/task-spec/SKILL.md, .claude/skills/implementation-check/SKILL.md, .claude/skills/acceptance-check/SKILL.md, tasks-p8.md --> <!-- context: 责任角色=QA；输入=全部长任务机制改动；输出=implementation-check 或等价自检结果；依赖关系=依赖 LT-01 到 LT-25；完成标准=自检确认未来新任务可生成恢复字段，P8 样例可清空聊天上下文后继续执行 --> <!-- order: 依赖全部机制改动完成 -->
- [ ] LT-27 执行长任务机制验收 <!-- files: docs/claude/standards.md, docs/claude/discipline.md, .claude/skills/, tasks-p8.md --> <!-- context: 责任角色=QA/PM；输入=自检结果、样例任务、任务系统文档；输出=acceptance-check 或等价验收结论；依赖关系=依赖 LT-26；完成标准=验收结论明确通过/不通过，并列出仍需人工确认的规则或迁移范围 --> <!-- order: 依赖长任务机制自检 -->
