...
- [ ] LT-08 更新 task-spec 拆分规则 <!-- files: .claude/skills/task-spec/SKILL.md --> <!-- context: 责任角色=PM；输入=长任务细颗粒度拆分偏好、现有 task-spec 执行规则；输出=更明确的拆分规则；依赖关系=依赖 LT-07；完成标准=skill 明确禁止把定义、实现、验证、索引、迁移混成同一条任务 --> <!-- order: 依赖 task-spec 字段要求 -->
