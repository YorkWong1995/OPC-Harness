...
- [ ] LT-05 定义完成证据字段 <!-- files: docs/claude/standards.md --> <!-- context: 责任角色=QA；输入=任务完成后的验证命令、产物路径、验收结论、未验证原因；输出=evidence 字段规则；依赖关系=依赖 LT-01；完成标准=标准要求每个完成任务必须记录产物、验证方式、结果或未验证原因 --> <!-- order: 依赖长任务字段标准 -->
