...
你必须先输出一个可解析的 JSON 对象，允许在 JSON 后补充 Markdown 说明，但 JSON 对象必须包含以下字段：

{
  "background": "背景与当前问题，非空字符串",
  "goal": "本次任务目标，非空字符串",
  "scope": ["包含范围，字符串数组"],
  "non_goals": ["不包含范围，字符串数组"],
...
- background 和 goal 不能为空
- scope、non_goals、acceptance_criteria、risks 即使为空也必须输出数组
- 不要只输出传统 PRD Markdown，否则工作流无法解析
...
OPTIONAL_ROLE_KEYWORDS = {
    "architect": ("架构", "系统设计", "设计方案", "模块边界", "接口边界", "数据结构", "技术方案", "技术取舍", "重构", "architecture"),
    "ops": ("部署", "发布", "上线", "环境", "运行检查", "运行风险", "监控", "回滚", "运维", "deploy", "release"),
    "growth": ("增长", "市场", "用户研究", "用户反馈", "竞品", "转化", "指标", "产品实验", "反馈假设", "growth", "research"),
}
...
角色含义：
- architect: 架构设计、系统设计、模块/接口边界、数据结构、复杂重构、技术方案取舍
- ops: 部署、发布、上线、环境、运行检查、监控、回滚、运维风险
- growth: 用户研究、用户反馈、增长、竞品、转化指标、产品实验、市场反馈假设
